# -*- coding: utf-8 -*-
"""
Created on Wed Jan 30 10:14:28 2019

@author: Kenneth

Solving for the power chagnes in the 4-directional quandrants of the phase speed spectrum

QUAD[0-4] quad[0]=NE, quad[0]=NW,quad[0]= SW, quad[0]=SE
...
for i in range(0,299):
    for j in range (0,299):
        ...
        if theta>=0:
            if theta>np.pi/2:
                quad[1]=quad[1]+10**data[i,j]
            else:
                quad[0]=quad[0]+10**data[i,j]
        else:
            if abs(theta)>=np.pi/2:
                quad[2]=quad[2]+10**data[i,j]
            else:
                quad[3]=quad[3]+10**data[i,j]
