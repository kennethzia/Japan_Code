...
#path='E:\JAPAN\Analysis24hr\AMTM_BandOH_WN_'
path='C:\Users\Kenneth\Desktop\MCM_AMTM_2017\Jun17-18\BandOH1hr\BandOH0_WN_'
...
#path='C:\Users\Kenneth\Desktop\MCM_AMTM_2017\Jun17-18\AMTM_Temp_WN_'
#
#files=glob.glob(path+'*.csv')
#files=natsorted(files)
...
plt.xlim(100,1)   
plt.loglog(x,f(x)/((x)**(3.0)*1.0*10**-6),label='AMTM-BandOH')  
#plt.loglog(x,f2(x)/((x)**(3.0)*1.0*10**-6),label='ASI')  

plt.title('Wavelength Analysis')
plt.xlabel('Wavelegnth (km)')
plt.ylabel('Power (1/km)')
#plt.loglog(x,(x)**(3.0)*1.0*10**-6,label='3-power')
#plt.loglog(x,(x)**(5.0/3.0)*1.0*10**-6,label='5/3-power')
plt.legend()
plt.tight_layout()
