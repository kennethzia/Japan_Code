pro Matsuda_FFT_V12_AMTM, img,dx=dx,dy=dy,dt=dt,$
...
DEVICE,filename='C:\Matsuda_FFT\AMTM_TEST0-59(3).ps'
!except=0
device, decomposed=0
;IRcam TEST DATA
;restore,'C:\Users\Kenneth\Desktop\IDL_FFT_CODE\Jul24-25\Not Broken Processed Star Removed Run 1.sav'
;AMTM TEST DATA
restore,'C:\Users\Kenneth\Desktop\test.sav'
...
if not (keyword_set(dt)) then dt=30.   ;Image time resolution (s);;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
dx=FLOAT(dx)
dy=FLOAT(dy)
;-----------------------Image Size--------------------------------------------------------;
...
if not (keyword_set(zpt)) then zpt=2.^8   ;Zero padding size in time dimension
...
xlen_hurf = LONG((xy2 - 1) * 0.5)
  ...
  name01=replicate(' ',30)
  IF not keyword_set(min1) THEN min1 =-11.5 ;minimum value of spectral density to be plotted
  IF not keyword_set(max1) THEN max1=-6.5 ;maximum value of spectral density to be plotted
...
;================================================================================
 ...
 save,FINAL_DATA, filename='C:\Matsuda_FFT\AMTM_TEST(3).sav'
